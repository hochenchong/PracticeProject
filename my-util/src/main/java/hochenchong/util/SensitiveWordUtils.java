...
import hochenchong.constant.SensitiveWordMatchType;
...
public class SensitiveWordUtils {
     ...
     * 默认的屏蔽字文件
     */
    private static final String SENSITIVE_WORD_FILE_PATH = "src/main/resources/static/sensitiveWord.txt";
    /**
    ...
    public static List<String> searchAllSensitiveWords(String text) {
        ...
        while (i < chars.length) {
            ...
            // 保险起见，排个序，从小到大
            lens = lens.stream().sorted().collect(Collectors.toList());
        ...
        // 可能会有重复，需要去重
        return allSensitiveWords.stream().distinct().collect(Collectors.toList());
    }
    ...
    public synchronized static void init() {
        ...
        File file = new File(SENSITIVE_WORD_FILE_PATH);

        if (!needReloadSensitiveWord(file)) {
            return;
        }
     ...
     */
    private static boolean needReloadSensitiveWord(File file) {
        try {
            if (!file.exists()) {
